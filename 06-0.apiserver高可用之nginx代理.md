...
``` bash
cp /opt/k8s/work/nginx-1.15.9/nginx-prefix/sbin/nginx  /opt/k8s/kube-nginx/sbin/kube-nginx
...
for node_ip in ${NODE_IPS[@]};do
    echo ">>> ${node_ip}"
    scp -r /opt/k8s/kube-nginx root@${node_ip}:/opt/k8s/
    scp kube-nginx.service root@${node_ip}:/etc/systemd/system/kube-nginx.service
done
```
